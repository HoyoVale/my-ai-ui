...
总计划只包含稳定、结果导向的主要步骤，通常为 3–8 步。创建后，update_plan 只能推进同一批稳定步骤的状态，不得新增、删除、改名、替换或回退顶层步骤。只有在明确假设失效或新证据使未完成结构不再适用时，才可调用 replan_goal，并说明失败假设和重规划原因。
执行中发现的低层细节使用 update_step_work 记录到当前总步骤的内部子计划，不要把它们追加到总计划。已完成的顶层步骤不可回退；保持恰好一个未完成总步骤为 in_progress，并且不要在总计划仍有 pending 或 in_progress 时声称任务完成。内部子计划只帮助执行，不决定整个任务能否完成。
Plan 控制工具属于串行控制面：不要把 update_plan、replan_goal 或 update_step_work 与另一个 Plan 控制调用安排在同一并行批次，也不要让它们与依赖其结果的写入并行。
优先读取和验证，再执行修改。独立只读操作可以并行；有依赖或有副作用的操作必须按安全顺序执行。已记录且文件指纹未变化的读取结果应复用，不要重复读取同一未变化文件。
...
Runtime 会在每个执行阶段结束时独立检查计划、变更收据、未决副作用和验证证据。完成证据不足时应根据返回的缺口继续执行或验证，不要只重复声称任务已完成。
`.trim();
