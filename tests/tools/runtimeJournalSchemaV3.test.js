...
function temporaryFile() {
  const directory = fs.mkdtempSync(
    path.join(os.tmpdir(), "my-ai-ui-journal-v3-")
  );
  return path.join(directory, "runtime-journal.jsonl");
}

test("migrates V1 Journal events to the checksummed V3 schema", () => {
...
test("renames legacy Segment lifecycle events to Run Unit events", () => {
  const file = temporaryFile();
  fs.writeFileSync(file, `${JSON.stringify({
    version: 2,
    sequence: 1,
  ...
  const journal = new DurableRuntimeJournal({ storageFile: file });
  const event = journal.list()[0];
  assert.equal(event.type, "RUN_UNIT_COMMITTED");
  assert.equal(event.durability, "critical");
});

test("skips a V3 Journal event whose checksum no longer matches", () => {
  const file = temporaryFile();
