...
describe("public commentary and tool batches", () => {
  it("groups related tool activity between public progress updates", () => {
    ...
    const before = store.recordCommentary({
      content: "我先检查消息活动的分组方式。",
      phase: "before_tools",
      objective: "检查活动分组"
    }, 110);
    ...
    const after = store.recordCommentary({
      content: "已经确认活动错误地按 task 聚合，接下来改为按 run 隔离。",
      phase: "after_tools"
    }, 140);
  ...
  it("publishes progress through an internal tool without consuming the tool budget", async () => {
    const store = new RunActivityStore({
      taskId: "task-2",
      runId: "run-2"
    });
    ...
    const result = await session.tools.report_progress.execute(
      {
        message: "我先核对当前事件源。",
        phase: "before_tools",
        objective: "核对事件源"
    ...
    assert.equal(session.getCallCount(), 0);
    assert.equal(
      store.snapshot().events.some((event) => event.type === "commentary"),
      true
    );
    assert.equal(
      store.snapshot().events.some(
        (event) => event.type === "tool" && event.tool?.name === "report_progress"
      ),
      false
    );
  });
