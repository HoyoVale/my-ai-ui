...
### 3.9 Tool V3：`delete_path` 与 `compare_files`
...
- 永远按破坏性操作逐次批准；
- 禁止删除工作区根目录、敏感路径、固定排除目录和符号链接；
- 非空目录递归删除必须显式声明；
- 支持 Dry-run、文件 Hash 前置条件、影响范围统计和文本删除 Diff；
- 删除前先原子移动到同目录隔离路径，捕获失败时尝试恢复；
- 异常遗留隔离路径不会被静默覆盖。

`compare_files`：

- 只读比较两个安全文本文件；
- 返回编码、换行、Hash、行数统计和有界 Diff；
- 不会作为实际文件改动写入 Conversation 活动。
...
- 用户可见根计划与内部子计划分离；
- 新增 `planState` Schema v2；
- `update_plan` 只维护根计划；
- 新增 `update_step_work` 维护当前根步骤的内部执行拆分；
- 旧扁平计划自动迁移；
- Assistant 消息与 Checkpoint 同时保存根计划和完整层级状态；
- Checkpoint 续跑恢复内部子计划；
- 普通 Plan Dock 只显示根计划；
- 任务终止判断只依赖根计划；
- 根步骤结束时自动归档尚未结束的内部子项。

详细说明见 `docs/PLAN_CORE_2_0_71.md`。

下一阶段为 Plan UI 2.0：优化根计划展示、计划修订提示，以及 Developer 模式下的内部子计划检查。
