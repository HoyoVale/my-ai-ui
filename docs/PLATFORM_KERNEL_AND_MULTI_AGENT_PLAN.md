...
| 版本 | 阶段 | 核心交付 | 硬性验收门 |
|---|---|---|---|
| 79 | Platform Kernel | PlatformRun、TaskGraph、AgentRun、Journal、Snapshot、租约、恢复、Completion Authority | 重启可恢复任务、依赖、Agent 和租约；无许可不能完成 Goal |
| 80 | Worktree Runtime | Git 快照、Worktree 创建、租赁、归档、回收、非 Git 降级 | 两个写 Agent 无法写同一目录；未合并改动不会丢失 |
| 81 | Multi-Agent | Supervisor、Planner、Explorer、Implementer、Tester、结构化交接和并发预算 | 独立任务可并行；依赖按序；失败可重试或重规划 |
