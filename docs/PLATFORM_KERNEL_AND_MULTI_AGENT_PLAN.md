...
> 基线：`my-ai-ui(81)`
> 文档状态：执行中
> 当前阶段：80–84 已完成；下一阶段为 85 — Visual Verification
...
| 82 | Integration & Review（已完成） | 集成 Worktree、Commit 队列、冲突任务、独立 Reviewer、安全发布 | Agent 不能自行合并；冲突不覆盖；已审查结果安全发布到用户工作区 |
| 83 | Local Platform（已完成） | 后台队列、暂停/恢复/取消/重试、预算、资源租约、状态 UI、Artifact 与日志中心 | 关闭窗口或异常退出后可观察并继续完整 Job |
| 84 | Verification Loop 3.0（已完成） | 失败分类、Replanner、证据关联、Reviewer 门、最终完成签名 | 集成、审查、逐条验收全部通过后才可完成 |
...
83 已完成。实现和操作边界见 [`LOCAL_PLATFORM_83.md`](LOCAL_PLATFORM_83.md)。
...
84 已完成。实现、签名边界和故障策略见 [`VERIFICATION_LOOP_84.md`](VERIFICATION_LOOP_84.md)。
...
- Worker 的真实变更 Commit 按任务创建顺序进入确定性 Integration Queue。
- 唯一 Integrator 在独立写 worktree 中顺序 cherry-pick，不依赖模型自行合并。
- Git 冲突会记录冲突文件、终止合并并恢复集成基线，不会静默覆盖任何一方。
- 集成结果形成独立 Commit、Artifact、摘要和绑定输入 Commit 的 digest。
- Reviewer 使用独立 AgentRun、独立模型调用和指定集成 Commit 的只读 worktree。
- Reviewer 必须返回结构化批准或拒绝；无效输出、越界写入和拒绝均不能通过完成门。
- 审查通过后，宿主程序比较用户工作区当前树与集成基线；期间发生并发修改时拒绝发布。
- 安全发布只应用 `baseline → integration` 差异，不切换用户分支、不修改真实 Git index、不替用户提交。
- 发布后再次验证工作树、分支和 index；只有与审查过的集成树一致时才记录 `published`。
- 只要存在 Worker 代码变更，Completion Authority 强制要求匹配同一集成 digest 的独立审查和发布状态。
- Input 与 Settings 的当前主模型统一读取会话绑定；新会话默认模型作为独立概念显示。
- 模型重命名或删除会协调所有会话快照；工作区增删会广播会话状态，避免跨窗口陈旧数据。

82 仍不包含后台 Job 管理、完整任务平台 UI、通知和外部 GitHub 适配器；它们分别属于 83 和 86。
