...
### 84.1 CI 与 Goal UI 修复

- Electron E2E 不再依赖固定延时或“新增模型会隐式切换当前会话”的偶然时序。测试先等待设置提交到主进程，再显式切换当前会话模型，并核对新会话继承的 `modelSnapshot`。
- Setting 顶栏公开稳定的保存状态测试契约，使 Windows 与 Linux Runner 都以实际提交完成为准。
- Goal 验收项的状态标记与操作区使用独立选择器，防止人工确认按钮被 18px 状态圆点规则压成竖排。
- Goal 操作栏移出滚动内容并固定在面板底部；长目标、可调 textarea 和多条验收标准不会再遮挡“清除 / 暂停 / 保存”。
- Electron E2E 增加按钮单行宽高、滚动区与页脚不重叠、页脚保持在面板内的实际 DOM 布局断言。
